...
async function callTencentTMT(text: string, source: string, target: string): Promise<string> {
  ...
  if (result.Response?.Error) {
    throw new Error(`TMT API Error: ${result.Response.Error.Code} - ${result.Response.Error.Message}`);
  }
...
function splitText(text: string, maxLength: number = 3000): string[] {
  ...
  for (const line of lines) {
    const potentialLength = current ? current.length + 1 + line.length : line.length;
...
// 保护不需要翻译的内容
function protectContent(text: string): { protected: string; placeholders: Map<string, string> } {
  ...
  protected_ = protected_.replace(/<\/?([A-Z][a-zA-Z0-9]*)(?:\s[^>]*)?\/?>/g, (match) => {
...
// 翻译单段文本
async function translateSegment(text: string, source: string, target: string): Promise<string> {
...
async function translatePost(options: TranslateOptions): Promise<void> {
  ...
  if (frontmatter.title) {
    translatedFrontmatter.title = await translateSegment(frontmatter.title, sourceLocale, targetLocale);
    console.log(`  title: ${translatedFrontmatter.title.substring(0, 50)}...`);
  }

  if (frontmatter.excerpt) {
    translatedFrontmatter.excerpt = await translateSegment(frontmatter.excerpt, sourceLocale, targetLocale);
  }

  if (frontmatter.category) {
    translatedFrontmatter.category = await translateSegment(frontmatter.category, sourceLocale, targetLocale);
  }
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const translated = await translateSegment(segments[i], sourceLocale, targetLocale);
...
async function main() {
  ...
  if (args.length < 3) {
    console.log('Usage: npx ts-node scripts/translate-post.ts <slug> <sourceLocale> <targetLocale>');
    console.log('Example: npx ts-node scripts/translate-post.ts scary-shawarma-kiosk en zh');
    process.exit(1);
  }
  ...
  if (!TENCENT_SECRET_ID || !TENCENT_SECRET_KEY) {
    console.error('Error: TENCENT_SECRET_ID and TENCENT_SECRET_KEY must be set in environment variables');
    process.exit(1);
  }
